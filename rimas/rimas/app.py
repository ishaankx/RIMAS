...
while True:
    ...
    if not monitoring_paused:  # Only proceed if monitoring is not paused
        ...
        # Process the frame and get the annotated version
        processed_frame = process_frame(frame)

        # Use imencode for more efficient display and handling
        _, buffer = cv2.imencode('.jpg', processed_frame)
        processed_frame = cv2.imdecode(buffer, cv2.IMREAD_COLOR)

    # Display the processed frame
    cv2.imshow("YOLOv8 Real-Time Webcam", processed_frame)
