...
while True:
    ...
    if not monitoring_paused:  # Only proceed if monitoring is not paused
        ...
        for result in results[0].boxes:
            ...
            if label == "away" or label == "using-phone":
                # Stop monitoring and start listening
                voice_command(f"{user_name}, why are you {label}? Please provide a reason.")
                listen_for_reason(label)  # Start listening in parallel
            elif label == "oncall" and label == "phone":
                voice_command(f"{user_name}, please put down the phone and get back to work!")
            elif label == "phone":
                voice_command(f"{user_name}, why are you on your{label}? Please provide a reason.")
                listen_for_reason(label)
            elif label == "idle":
